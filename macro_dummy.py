...
def main():
    ...
    while interface.is_coupling_ongoing():
        ...
        # create randowm scalar data
        print('read scalar data: ', read_scalar_data)
        write_scalar_data[:] = np.random.randint(0, 10, nv)
        for i in range(nv):
            for d in range(
                interface.get_data_dimensions(
                    read_mesh_name, "micro-vector-data")
            ):
